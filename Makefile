...
LIB_MODULES = compiler runtime
TEST_MODULES = compilertest runtimetest
BIN_MODULES = degc degrun

compiler_DEPENDENCIES = runtime
compiler_LIBRARIES = boost_filesystem boost_system
...
clean:
	...
	-@rm $(SRC_PATH)/compiler/grammar/lexer.cpp $(SRC_PATH)/compiler/grammar/parser.cpp 2> /dev/null || true
	-@rm $(SRC_PATH)/compiler/grammar/parser.hpp $(SRC_PATH)/compiler/grammar/parser.output 2> /dev/null || true
...
$(SRC_PATH)/compiler/grammar/parser.cpp $(SRC_PATH)/compiler/grammar/parser.hpp: $(SRC_PATH)/compiler/grammar/parser.y
	bison --output=$(SRC_PATH)/compiler/grammar/parser.cpp $(SRC_PATH)/compiler/grammar/parser.y
